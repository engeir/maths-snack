...
class Circle:
    ...
    def draw(self, screen, factor, pt, lines, circle):
        if circle:
            pg.draw.circle(screen, cf.WHITE, (int(cf.SCREEN_WIDTH / 2), int(cf.SCREEN_HEIGHT / 2)), self.radius, 1)
        for i in range(pt):
            angle = np.pi * i / pt
            ...
            x_ = ((x1 - x0) * (.5 + np.cos(angle + factor) / 2) + x0)
            y_ = ((y1 - y0) * (.5 + np.cos(angle + factor) / 2) + y0)
