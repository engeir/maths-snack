...
class Circle:
    ...
    def draw(self, screen, wheels, theta, draw, version):
        x0, y0 = int(cf.SCREEN_WIDTH * .75), int(cf.SCREEN_HEIGHT /
                                               2 + cf.SCREEN_HEIGHT * .04)
        # Normalize radius
        ...
        for w in wheels:
            ...
            pg.draw.circle(screen, cf.R7, (int(x[-1] - cf.SCREEN_WIDTH * .5), int(y[-1])), r, 1)
            x_ = int(r * np.cos(theta * omega + offset)) + x[-1]
            y_ = int(r * np.sin(theta * omega + offset)) + y[-1]
            x = np.r_[x, x_]
            y = np.r_[y, y_]
            p = np.c_[x - int(cf.SCREEN_WIDTH * .5), y]
            pg.draw.lines(screen, cf.WHITE, False, p, 1)
        if self.last_x is not None:
            if draw:
                try:
                    pg.draw.line(screen, cf.WHITE, (self.last_x, self.last_y), (x[-1], y[-1]), 4)
                except Exception:
                    pass
        self.last_x = x[-1]
        self.last_y = y[-1]
...
class Wheel:
    def __init__(self):
        ...
        self.t = cf.SCREEN_HEIGHT / 2 + cf.SCREEN_HEIGHT * .05 - self.rad
        self.b = cf.SCREEN_HEIGHT / 2 + cf.SCREEN_HEIGHT * .05 + self.rad
        self.p = [(self.l, self.t), (self.r, self.t), (self.r, self.b), (self.l, self.b)]

    def create_wheel(self, wheel):
        if wheel == 'w1':
            w1 = np.array([[1, - 2 / 4, 0], [.5, 5 / 4, 0], [1/4, 19 / 4, 0]])
            self.w = w1
        elif wheel == 'w2':
            w2 = np.array([[1, 0.8, 0.4, 0.2, 0.4, 0.2],
                           [1, 10, - 17, - 26, 28, 37],
                           [0, - np.pi / 2, - np.pi / 2, 0, 0, np.pi / 2]]).T
            self.w = w2
        elif wheel == 'w3':
            w3 = np.array([[1, 1 / 2, 1 / 4],
                           [1, 4, 31],
                           [0, 0, 0]]).T
            self.w = w3
        elif wheel == 'w4':
            w4 = np.array([[1, 1 / 2, 1 / 4],
                           [1, 3, 80],
                           [0, 0, 0]]).T
            self.w = w4
        elif wheel == 'w5':
            w5 = np.array([[1, .4, .3,        .3, .1, .05],
                           [1,  3, 5,       - 10,  3,  45],
                           [0,  np.pi,  0, np.pi / 2,  0,   0]]).T
            self.w = w5
        if wheel == 'w0':
            pass
        else:
    ...
    def remove_adjacent(self):
        ...
        for i, j in self.path[:-5, :]:
            if i_last is None and j_last is None:
                ...
                dist = abs((i*i+j*j)**.5 - (i_last*i_last+j_last*j_last)**.5)
                if dist < 3:
