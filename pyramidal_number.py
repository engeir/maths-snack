...
def hept_pyramid(s, n):
    """Make a pyramid with a polygonal shaped base.

    Arguments:
        s {int} -- polygonal shape
        n {int} -- height/side length of the pyramid
...
def hept(s, n):
    ...
    Arguments:
        s {int} -- polygonal shape
        n {int} -- side length of the polygon
...
try:
    while True:
        if h == pyr:
            print('Success! Gonal number = ' + str(gonal_number) + ' Pyramid count = ' + str(pyr_c) + ' Polygon count = ' +
                  str(h_c) + ' Canon balls = ' + str(h))
