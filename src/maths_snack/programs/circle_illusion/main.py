...
import maths_snack.programs.circle_illusion.config as cf
from maths_snack.programs.circle_illusion.circle import Circle
...
class Sim:
    ...
    def menu(self):
        if self.choose_version is None:
            self.text_choose.type(
                self.screen,
                cf.WHITE,
                text=f"Points: {int((self.x / cf.SCREEN_WIDTH) * 100)}",
            )
        elif self.choose_version == "circ":
            self.text_choose.type(self.screen, cf.GREEN, text=f"Points: {self.num}")
        if self.choose_version is not None and self.wait:
    ...
    def write(self):
        ...
        self.text_points = Text(
            ...
            cf.SCREEN_HEIGHT / 2 - cf.SCREEN_HEIGHT * 0.24,
            loc="left",
        )
    ...
    def circle_sim(self):
        ...
        line_str = ["Show", "Hide"]
        self.text_lines.type(
            self.screen, cf.WHITE, text=f'{line_str[self.lines]} lines: "L"'
        )
        self.text_circ.type(
            self.screen, cf.WHITE, text=f'{line_str[self.circle]} circle: "C"'
        )
