...
import maths_snack.programs.dragon_curve.config as cf
from maths_snack.programs.dragon_curve.dragon import Dragon
        ...
        for event in pg.event.get():
            ...
            if self.wait:
                if event.type == pg.MOUSEBUTTONUP:
                    if (
                        self.text_auto.ell < self.x
                        and self.x < self.text_auto.r
                        and self.text_auto.t < self.y
                        and self.y < self.text_auto.b
                    ):
                        self.auto = True
                    elif (
                        self.text_manual.ell < self.x
                        and self.x < self.text_manual.r
                        and self.text_manual.t < self.y
                        and self.y < self.text_manual.b
                    ):
                        self.auto = False
                    if self.auto is not None:
                        self.check_auto_not_none()

    def check_auto_not_none(self):
        if (
            self.text_2.ell < self.x
            and self.x < self.text_2.r
            and self.text_2.t < self.y
            and self.y < self.text_2.b
        ):
            self.dragon_size = 1
        if (
            self.text_3.ell < self.x
            and self.x < self.text_3.r
            and self.text_3.t < self.y
            and self.y < self.text_3.b
        ):
            self.dragon_size = 2
        if (
            self.text_5.ell < self.x
            and self.x < self.text_5.r
            and self.text_5.t < self.y
            and self.y < self.text_5.b
        ):
            self.dragon_size = 3
        if (
            self.text_40.ell < self.x
            and self.x < self.text_40.r
            and self.text_40.t < self.y
            and self.y < self.text_40.b
        ):
            self.dragon_size = 4

    def write(self):
        self.text_esc_r = Text(
            'If in doubt, press "r". Press "esc" to quit.',
            cf.SCREEN_WIDTH / 2,
            cf.SCREEN_HEIGHT / 2 + cf.SCREEN_HEIGHT * 0.3,
        )
        self.text_press_n = Text(
            'Press "n".', cf.SCREEN_WIDTH / 2, cf.SCREEN_HEIGHT / 2
        )
        self.text_press_n_a_lot = Text(
            'Press "n". Repeatedly.', cf.SCREEN_WIDTH / 2, cf.SCREEN_HEIGHT / 2
        )
        self.text_a_or_m = Text(
            "Automatic or Manual Simulation?",
            cf.SCREEN_WIDTH / 2,
            cf.SCREEN_HEIGHT / 2 - cf.SCREEN_HEIGHT * 0.35,
        )
        self.text_auto = Text(
            "Automatic",
        ...
        self.text_dragon_size = Text(
            "Choose the length of the lines in your Dragon.",
            cf.SCREEN_WIDTH / 2,
            cf.SCREEN_HEIGHT / 2 - cf.SCREEN_HEIGHT * 0.05,
        )
        self.text_2 = Text(
            "2",
            cf.SCREEN_WIDTH / 2 - cf.SCREEN_WIDTH * 0.2,
        ...
        self.text_3 = Text(
            "3",
            cf.SCREEN_WIDTH / 2 - cf.SCREEN_WIDTH * 0.07,
            cf.SCREEN_HEIGHT / 2 + cf.SCREEN_HEIGHT * 0.1,
        )
        self.text_5 = Text(
            "5",
