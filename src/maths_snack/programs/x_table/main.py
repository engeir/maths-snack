...
import maths_snack.programs.x_table.config as cf
from maths_snack.programs.utils.my_texts import Text
from maths_snack.programs.x_table.x_table_calc import TimesTable
...
class Sim:
    ...
    def write(self):
        ...
        self.text_auto = Text(
            "Auto (increasing number of points)",
            cf.SCREEN_WIDTH / 2 - cf.SCREEN_WIDTH * 0.4,
            cf.SCREEN_HEIGHT / 2 - cf.SCREEN_HEIGHT * 0.25,
            loc="l",
        )
        self.text_max_auto = Text(
            "Auto (constant number of points)",
            cf.SCREEN_WIDTH / 2 - cf.SCREEN_WIDTH * 0.4,
            cf.SCREEN_HEIGHT / 2 - cf.SCREEN_HEIGHT * 0.15,
            loc="l",
        )
        self.text_ctrl = Text(
            "Full control",
        ...
        self.text_max_factor = Text(
            "Set the max factor value.",
            cf.SCREEN_WIDTH / 2,
        ...
        self.text_factor = Text(
            "",
            cf.SCREEN_WIDTH / 2 - cf.SCREEN_WIDTH * 0.45,
            cf.SCREEN_HEIGHT / 2 - cf.SCREEN_HEIGHT * 0.4,
            loc="l",
        )
        self.text_points = Text(
            "",
            cf.SCREEN_WIDTH / 2 - cf.SCREEN_WIDTH * 0.45,
            cf.SCREEN_HEIGHT / 2 - cf.SCREEN_HEIGHT * 0.32,
            loc="l",
        )
    ...
    def full_control(self):
        ...
        self.text_factor.type(self.screen, cf.WHITE, text=f"Factor: {round(h, 2)}")
        self.text_points.type(self.screen, cf.WHITE, text=f"Points: {int(w)}")
    ...
    def full_auto(self):
        ...
        self.text_points.type(self.screen, cf.WHITE, text=f"Points: {int(self.num)}")
