...
class TextInput:
    ...
    def __init__(
        ...
        cursor_color: tuple[int, int, int] = (0, 0, 1),
        repeat_keys_initial_ms: int = 400,
        repeat_keys_interval_ms: int = 35,
        ...
        Args:
            ...
            font_family: name or list of names for font (see pygame.font.match_font for precise format)
            font_size: Size of font in pixels
            antialias: Determines if antialias is applied to font (uses more processing power)
            text_color: Color of text (duh)
            cursor_color: Color of cursor
            repeat_keys_initial_ms: Time in ms before keys are repeated when held
            repeat_keys_interval_ms: Interval between key press repetition when held
            max_string_length: Allowed length of text
            password: password
        """
        ...
        self.keyrepeat_counters: dict[Any, int] = {}
        ...
        for event in events:
            if event.type == pygame.KEYDOWN:
                ...
                if event.key not in self.keyrepeat_counters:
                    if (
                        not event.key == pl.K_RETURN
                    ):  # Filters out return key, others can be added as necessary
