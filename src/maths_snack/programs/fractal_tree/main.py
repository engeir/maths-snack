...
import maths_snack.programs.fractal_tree.config as cf
from maths_snack.programs.fractal_tree.tree import Tree
...
class Sim:
    ...
    def menu(self):
        ...
        self.text_choose_a.type(
            self.screen, cf.WHITE, text=f"{self.angle[1]}", size=cf.TXT_BIG
        )
        self.text_choose_r.type(
            self.screen, cf.WHITE, text=f"{self.reproduction[1]}", size=cf.TXT_BIG
        )
        self.text_choose_s.type(
            self.screen, cf.WHITE, text=f"0.{self.scale[1]}", size=cf.TXT_BIG
        )
        ...
        for i, t in enumerate(self.text_choose_n):
            if t.overlay(self.x, self.y):
                ...
                break
        if (
            self.reproduction[1] is not None
            and self.angle[1] is not None
            and self.scale[1] is not None
            and self.wait
        ):
            self.text_version.type(self.screen, cf.WHITE, size=cf.TXT_BIG)
    ...
    def write(self):
        ...
        self.text_choose_a = Text(
            f"{self.angle[1]}",
            cf.SCREEN_WIDTH / 2 + cf.SCREEN_WIDTH * 0.1,
            cf.SCREEN_HEIGHT / 2 - cf.SCREEN_HEIGHT * 0.1,
            loc="l",
        )
        self.text_choose_r = Text(
            f"{self.reproduction[1]}",
            cf.SCREEN_WIDTH / 2 + cf.SCREEN_WIDTH * 0.1,
            cf.SCREEN_HEIGHT / 2 - cf.SCREEN_HEIGHT * 0.25,
            loc="l",
        )
        self.text_choose_s = Text(
            f"{self.scale[1]}",
            cf.SCREEN_WIDTH / 2 + cf.SCREEN_WIDTH * 0.1,
            cf.SCREEN_HEIGHT / 2 + cf.SCREEN_HEIGHT * 0.05,
            loc="l",
        )
        self.text_choose_n = [self.text_angle, self.text_repr, self.text_scale]
        ...
        for event in pg.event.get():
            if event.type == pg.KEYDOWN:
                ...
                if event.key == pg.K_n:
                    ...
                    self.wait = False
                    self.tree.set_vars(
                        self.reproduction[1], self.angle[1], self.scale[1]
                    )
